...
interface DayDifferent(TInputA, TInputB, TResult, string path)
{
    final void runA()
    {
        auto t = Timer();
        t.start;
        auto data = parseDataA(readFile, &t);
        t.lap("Parsing data");
        auto answerA = problemA(data, &t);
        t.stop;
        t.lap("Total");
    ...
    final void runB()
    {
        auto t = Timer();
        t.start;
        auto data = parseDataB(readFile, &t);
        t.lap("Parsing data");
        auto answerB = problemB(data, &t);
        t.stop;
        t.lap("Total");
    ...
    TInputA parseDataA(string data, Timer* timer);
    TResult problemA(TInputA data, Timer* timer);
    TInputB parseDataB(string data, Timer* timer);
    TResult problemB(TInputB data, Timer* timer);
}
